...
class GroupeVenteController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (isset($create_groupe)) {
            return redirect()->route('ventes.groupe_ventes.index')->with(['success' => "Vous venez d'enregistrer ".$create." groupe(s)."]);
        }else {
            return redirect()->route('ventes.groupe_ventes.index')->with(['info' => "Aucun groupe ajouté."]);
        }
    }
    ...
    public function destroy(string $id)
    {
        if ($groupe = GroupeVente::find($id)) {
            ...
            return redirect()->route('ventes.groupe_ventes.index')->with('success', 'Groupe supprimé avec succès.');
        } else {
            return redirect()->route('ventes.groupe_ventes.index')->with('error', 'Groupe non trouvé.');
        }
    }
}
